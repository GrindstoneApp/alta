...
export class JwtInterceptor implements HttpInterceptor {
  ...
  private handle500Error(request: HttpRequest < any >, next: HttpHandler, err: any): Observable < any > {
    if(err.error.errors) {
      if(err.error.errors.toString().includes('error retrieving calling user')) {
        // Token Expired
  ...
  private handle401Error(request: HttpRequest < any >, next: HttpHandler, err: any): Observable < any > {
    if(err.error) {
       ...
       } else if(err.error.message.toString().includes('expired bearer token')) {
            // Token Expired
            console.error('Token Expired on call, queuing all requests until new issued');
